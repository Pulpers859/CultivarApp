...
1. **Xcode Project File** — The `.xcodeproj` file is not included (generated by Xcode). Create a new SwiftUI project, delete the default files, and copy these source files in. Set minimum iOS deployment to **iOS 17** (required for SwiftData and `.onChange(of:)` two-parameter syntax).
