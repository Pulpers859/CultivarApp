...
- **Session 8** (April 6, 2026): Added `docs/safety/DATA_SAFETY_CHECKLIST.md` with a release-time data retention checklist (bundle ID stability, SwiftData schema change rules, migration requirements, smoke tests, and recovery guidance) to reduce risk of accidental data loss during future updates.
...
- **Session 13** (June 17, 2026): Cleaned up the repo root without moving runtime files. Added a root `README.md`, created `docs/` for support material, moved handoff/safety/template docs under `docs/`, and updated stale documentation that still described a pre-`Cultivar.xcodeproj` state.
