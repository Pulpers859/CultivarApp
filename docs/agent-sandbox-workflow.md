...
```powershell
.\tools\New-AgentSandbox.ps1 -Name reminder-refactor
```

Review the sandbox diff, then integrate only selected changes back into the main checkout:

```powershell
git -C C:\Dev\.agent-sandboxes\CultivarApp\reminder-refactor diff
```

Remove the sandbox when finished:

```powershell
.\tools\Remove-AgentSandbox.ps1 -NameOrPath reminder-refactor
```

## Use A Sandbox For

- SwiftData schema, migration, backup, import, or recovery experiments.
- Reminder/date math changes where a bad edit could affect user data.
...
The sandbox is for isolation, not final delivery. Final validation, commit, and push happen from `C:\Dev\CultivarApp` on `main`.
